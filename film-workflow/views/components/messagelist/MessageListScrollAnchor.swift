...
/// Tracks whether the list is "following the bottom".
///
/// Ported from rxcode's `MessageList` package. Pure and testable: it takes
/// scroll geometry samples and decides whether new content should pull the view
/// down with it.
nonisolated struct MessageListScrollAnchor: Equatable {
    ...
    mutating func apply(
        ...
        // only run for a genuine user scroll. A tall card (a big tool result,
        // a long caption list) that lays out in one frame leaves
        // `distanceFromBottom` huge until the throttled/async scroll-to-bottom
        // executes; a layout settle that lands in that window would otherwise
        // recompute `isNearBottom` to false even though the user never
        // scrolled — which then makes the pending auto-scroll bail and strands
        // the view above the bottom. Gating on `isUserDriven` keeps the anchor
        // sticky through that settle while still letting a deliberate scroll up
        // release it.
