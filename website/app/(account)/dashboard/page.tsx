...
export default async function DashboardPage() {
  ...
  const [balance, devices, [spent]] = await Promise.all([
    ...
    db.select({ points: sql<number>`coalesce(sum(${usageEvents.chargedPoints}), 0)`.mapWith(Number) }).from(usageEvents).where(and(eq(usageEvents.userId, user.id), eq(usageEvents.status, "settled"))),
  ]);
